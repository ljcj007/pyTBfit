...
def fit(Edata):
    ...
    K=np.append(K, np.append(kxG_M,kxG_M))
...
def findfit(i1):
    data=np.loadtxt("3/0-"+str('%.2f' % i1)+"-BAND.dat", delimiter=None)
    press = np.zeros(6, dtype=np.int)
    a=np.zeros((300, 6))
    index=0
...
def mytry(i1):
    cha=i1
    data=np.loadtxt("./BAND-1-3.dat", delimiter=None)
...
def fig(i):
    ...
    plt.grid(axis="y")    
    plt.margins(0, 0)
    plt.savefig('./three.eps',dpi=600,format='eps')
    plt.show()

...
def findfit1(data):    
    ...
    for i1 in range(np.shape(real)[1]):
        if i1%2==1:
            real[:, i1]=real[::-1, i1]
    Edata = np.append(real[100:150, 15], real[100:150, 16])
    kxG_M = np.linspace(0,100,100)
    popt, pcov = curve_fit(engy1, kxG_M, Edata, maxfev=50000, p0=(2.9,2.9,2.9,0.8))
...
def findfit(data):    
    ...
    popt, pcov = curve_fit(engy, kxG_M, Edata, maxfev=50000, p0=(2.7,2.7,2.7,1.0))
    perr = np.sqrt(np.diag(pcov))
    print(popt)
    print(perr)
    return popt,perr

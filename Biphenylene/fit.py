...
def findfit(data):    
    ...
    popt, pcov = curve_fit(engy, kxG_M, Edata, maxfev=50000, p0=(-2.7,-2.7,-2.7,-2.8))
...
def findfit1(data):    
    ...
    popt, pcov = curve_fit(engy1, kxG_M, Edata, maxfev=50000, p0=(2.7,2.7,2.7,2.8))
    perr = np.sqrt(np.diag(pcov))
    print(popt)
    print(perr)
    return popt,perr
...
def energy_bond(data):
    #tfit,perr=findfit1(data)
    tfit=[2.75,2.95,2.95,2.95]
    hw=[]
    for i in range(250):
        if i<50:
            kx = np.pi*(i)/49
